...
def retrieve_all_products():
    ...
    conn.close()
    return products
# def get_cart_item_by_email(email):
    # conn = sqlite3.connect('ecommerce.db')
    # c = conn.cursor()
    # cart_item = c.execute("SELECT cart FROM accounts WHERE email=(?)",(email,)).fetchone()
    # conn.commit()
    # conn.close()
    # items_id = cart_item.split(' ')
    # items = []
    # for id in items_id:
    #     items.append(retrieve_products_by_id(id))
    # return items
